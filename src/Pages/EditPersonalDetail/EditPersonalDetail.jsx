...
const EditPersonalDetail = () => {
  ...
  const userDetails = async () => {
    ...
    //console.log(data?.data?.UserDetails?.bio);
    const bioInput = document.querySelector("#bio");
    // setDrinkPreference(data?.UserDetails.drink);
    const bioTextarea = document.querySelector(".bio");
    const userDetails = data?.data?.UserDetails;
    if (userDetails) {
      document.querySelector(".height").value = userDetails.height;
      document.querySelector(".weight").value = userDetails.weight;
      const dobInput = document.querySelector(".dob");
      const dob = new Date(userDetails.dob).toISOString().split("T")[0];
      setDOB(dob);
      dobInput.value = dob;
      ...
      document.querySelector(".religion").value = userDetails.religion;
      document.querySelector(".community").value = userDetails.community;
      document.querySelector(".mother-tongue").value =
        userDetails.mother_tongue;
      document.querySelector(".marital-status").value =
        userDetails.marital_status;
      document.querySelector(".food-preference").value =
        userDetails.food_preference;
      document.querySelector(".smoke").value = userDetails.smoke;
      document.querySelector(".drink").value = userDetails.drink;
      document.querySelector("#bio").value = userDetails.bio;
      document.querySelector(".gender").value = userDetails.gender;
      setCountry(userDetails.country);
      loadStates(userDetails.country);
      setState(userDetails.state);
      loadCities(userDetails.state);
      setHome_town(userDetails.home_town);
    }
    ...
    <div>
      <div className="r_login__wrapper1">
        <div className="r_login">
          ...
          <div className="d-flex gender_state" style={{ gap: "15px" }}>
            <input
              placeholder="DOB"
              type="Date"
              className="dob gender_state"
              id="dob"
              max={getMaxDate()}
          ...
          <div className="d-flex gender_state" style={{ gap: "15px" }}>
            ...
            <input
              ...
              type="text"
              style={{width: "100%"}}
              className="weight gender_state text-center"
            />
          </div>
          {/* country */}
          {/* <div className="gender_state">
